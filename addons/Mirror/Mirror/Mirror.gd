...
func _initialize_references() -> void:
	...
	cam = get_node_or_null("MirrorContainer/SubViewport/Camera3D")
	mirror = get_node_or_null("MirrorContainer/MeshInstance3D")
	viewport = get_node_or_null("MirrorContainer/SubViewport")
	if cam and mirror and viewport:
		_initialized = true


func _process(delta):
	# Re-attempt initialization if not yet complete (handles tool script scene changes)
	if not _initialized:
		_initialize_references()
		if not _initialized:
			return
