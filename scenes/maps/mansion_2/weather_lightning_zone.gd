class_name WeatherLightningZone
extends Area3D
...
@export_enum("full_flash", "partial_flash", "no_flash") var flash_tier: int = FlashTier.FULL_FLASH
...
func get_flash_multiplier(full_flash_multiplier: float, partial_flash_multiplier: float, no_flash_multiplier: float) -> float:
	match flash_tier:
		FlashTier.PARTIAL_FLASH:
			return partial_flash_multiplier
		FlashTier.NO_FLASH:
			return no_flash_multiplier
		_:
			return full_flash_multiplier


func get_flash_tier_name() -> StringName:
	match flash_tier:
		FlashTier.PARTIAL_FLASH:
			return &"partial_flash"
		FlashTier.NO_FLASH:
			return &"no_flash"
		_:
			return &"full_flash"
...
func _shape_contains_local_point(shape: Shape3D, local_point: Vector3) -> bool:
	...
	if not _warned_unsupported_shape:
		...
		push_warning("WeatherLightningZone supports BoxShape3D and SphereShape3D volumes in v1: %s" % name)
	return false
