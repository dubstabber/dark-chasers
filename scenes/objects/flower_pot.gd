...
func take_damage(dmg: int) -> void:
	if not is_destroyed:
		...
		if health <= 0:
			...
			Services.utils.play_sound(catalog.pot_break_sound, get_parent(), position)
			is_destroyed = true
			...
			for i in 10:
				var pot_scrap = catalog.scrap_scene.instantiate()
			...
			for i in [6, 10].pick_random():
				var circle_scrap = catalog.scrap_scene.instantiate()
			...
			for i in [4, 6].pick_random():
				var small_scrap = catalog.scrap_scene.instantiate()
			...
			for i in 10:
				var grass_scrap = catalog.scrap_scene.instantiate()
