...
func take_damage(dmg: int):
	...
	if health <= 0:
		...
		Services.utils.play_sound(catalog.wood_break_sound, get_parent(), position)
		for i in 4:
		...
		var big_scrap = catalog.scrap_scene.instantiate()
		...
		for i in [7, 8].pick_random():
			var white_scrap = catalog.scrap_scene.instantiate()
