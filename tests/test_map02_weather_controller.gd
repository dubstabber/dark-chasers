...
func _test_map02_scene_weather_authoring() -> void:
	...
	# MAP02 may not currently author any `WeatherRainVisualZone` areas in the scene.
	# The weather controller still needs the particle nodes under `RainVisualZones`.
	assert(scene_text.contains("[node name=\"RainParticles\" type=\"GPUParticles3D\" parent=\"WeatherController/WeatherZones/RainVisualZones\"]"), "MAP02 scene should include rain particle FX under RainVisualZones")
	assert(scene_text.contains("[node name=\"RainParticles2\" type=\"GPUParticles3D\" parent=\"WeatherController/WeatherZones/RainVisualZones\"]"), "MAP02 scene should include secondary rain particle FX under RainVisualZones")
...
func _test_rain_visual_zone_can_be_discrete() -> void:
	var controller := _build_controller(false)
	_prime_controller(controller)
	_attach_player(controller, Vector3.ZERO)
	_attach_rain_visual_zones(
	...
	controller.free()
	print("✓ Rain visual zones still drive visible rain")
